...
- MR 필터링 시 반드시 String(값).trim() 후 "실무" 포함 여부로 체크 (셀 공백/줄바꿈 대비)
- 실무 MR은 약 6명(박병환, 임요묵, 윤석준, 이근우, 김태화, 유광현)

## 시트명: 고객마스터 (총 1,001행, 헤더 포함 → 데이터 1,000행)
...
- 잠재등급: "A"(핵심), "B"(성장), "C"(유지) → trim 처리 필수
- 최근반응: "긍정", "보통", "부정"
- 최근활동일/다음활동예정일은 날짜 또는 빈칸 → new Date()로 파싱, 빈 경우 null 처리
...
## 상단 KPI 카드 (4개 가로 배열)
1. 전체 담당 거래처 수 (고객마스터 데이터 행 수 → 1,000건 예상)
2. 이번 달 총 활동건수 (활동로그에서 활동일자 기준 이번 달 집계)
3. 잠재등급 A인 거래처 수 (핵심 고객)
4. 다음활동예정일이 오늘~7일 이내인 거래처 수
- 각 카드: 둥근 모서리(border-radius:12px), 그림자, 숫자 2.4rem 이상, 상단 컬러 라인(파랑/초록/주황/빨강)

## MR 배치 현황 카드 그리드
- 조직도및MR 시트의 실무 MR 전원을 카드 형태로 그리드 배열 (auto-fill, minmax(210px, 1fr))
- 각 카드 표시 내용:
  - 상단: MR이름(볼드) + 직급 배지
  - 📍 담당 사무소
  - 3열 통계: 담당거래처수 | 이번달활동수 | A등급수
  - 활동달성률 = (이번달활동수 합계 / 월목표활동수 합계) × 100 → 퍼센트 진행바
    - 80% 이상: 초록(#10B981), 50~79%: 주황(#F59E0B), 50% 미만: 빨강(#EF4444)
  - 방문 지연 배지: "N건 지연"(빨강) 또는 "지연없음"(초록)
- 카드 호버 시 살짝 떠오르는 효과 (transform: translateY(-2px))

## 고효율 집중 필요 구역 분석
- 조건: 잠재등급이 A 또는 B이고, 잠재점수 60점 이상인 거래처 중 아래 중 하나라도 해당
  - 이번달활동수가 0건
  - 다음활동예정일이 오늘 기준 7일 이상 지연
- 정렬: A등급 우선 → 잠재점수 높은 순 → 지연일수 많은 순
- 최대 30건 표시
- 섹션 상단 노란 경고 배너(#FFFBEB 배경): "잠재등급 A·B, 잠재점수 60점 이상인데 이번 달 활동이 0건이거나 방문이 7일 이상 지연된 거래처입니다. 소장의 즉각적인 리소스 배치가 필요합니다."
- 컬럼: 거래처명 | 고객명 | MR | 사무소 | 잠재등급 | 잠재점수 | 현재단계 | 이번달활동수 | 지연일수 | 사유
  - 사유 자동 분류: "이번달활동없음"(주황 배지) / "방문지연"(빨강 배지) / "활동없음+지연"(보라 배지)

## 현재단계 파이프라인 (1~5단계)
- 각 단계별 거래처 수를 카드로 가로 5분할 표시
- 단계 이름: 1=초기탐색, 2=관계형성, 3=검토시험, 4=정기처방, 5=핵심고객
- 단계별 파스텔 배경색 (1:파랑, 2:초록, 3:노랑, 4:주황, 5:보라)
- 카드 클릭 시 하단 전체 거래처 리스트 필터링

## MR별 성과 비교 테이블
- 컬럼: MR이름 | 담당 | 이번달활동(가로 바 차트 포함) | A등급 | 4~5단계 | 평균점수
- 행 클릭 시 하단 거래처 리스트를 해당 MR로 필터

## 방문 지연 거래처 리스트
- 다음활동예정일이 오늘보다 지난 거래처 (지연일수 내림차순 정렬)
- 컬럼: 거래처명 | MR | 현재단계 | 잠재등급 | 다음활동예정일 | 지연일수(빨간 배지)

## 전체 거래처 리스트 (검색/필터 + 페이지네이션)
- 필터: MR 드롭다운 | 현재단계 드롭다운 | 잠재등급 드롭다운 | 거래처명/고객명 검색 | 초기화 버튼
- 페이지당 20건, 하단에 페이지네이션 (‹ 1 2 3 … ›)
- 컬럼: 거래처명 | 고객명 | MR | 현재단계 | 잠재등급 | 잠재점수 | 최근활동일 | 다음활동예정일

# 디자인 요구사항
- 색상: 네이비 헤더(#1A2E5A) + 흰 배경(#F8FAFC) + 파란 포인트(#2563EB)
- 폰트: Noto Sans KR (구글 폰트 링크 포함, 300~800 weight)
- 섹션 제목 왼쪽에 파란 세로 바 포인트
- 배지 스타일: border-radius:20px, 파스텔 배경 + 진한 글자
- 반응형: 1024px / 640px / 400px 3단계 브레이크포인트
  - 640px 이하: KPI 2열, 파이프라인 3열, MR 배치 2열
  - 400px 이하: KPI/파이프라인/MR배치 모두 1열

# 기술 요구사항 (매우 중요 - 이 부분 지키지 않으면 SyntaxError 발생)

## Apps Script 기본 구조
- doGet() 함수로 웹앱 구현, HtmlService.createHtmlOutput() 으로 HTML 반환
- setXFrameOptionsMode(HtmlService.XFrameOptionsMode.ALLOWALL) 설정 (iframe 임베드 허용)
- 상단에 const SPREADSHEET_ID = ''; 변수로 분리
- SpreadsheetApp.openById(SPREADSHEET_ID).getSheetByName('시트명').getDataRange().getValues() 로 데이터 로드

## 데이터 주입 안전 처리 (필수)
- JSON.stringify(data) 결과를 <script> 태그에 주입할 때 반드시 이스케이프:
  .replace(/</g, '\\\\u003c')
  .replace(/>/g, '\\\\u003e')
  .replace(/&/g, '\\\\u0026')
  .replace(/\\u2028/g, '\\\\u2028')
  .replace(/\\u2029/g, '\\\\u2029')
- 셀 데이터에 < > & 가 포함돼도 스크립트가 깨지지 않도록 방어

## 이벤트 바인딩 (중요)
- 절대 HTML 문자열에 onclick="func('문자값')" 같은 inline 이벤트를 넣지 말 것
  (Apps Script 템플릿 리터럴이 \\' 를 ' 로 변환시켜 SyntaxError 발생)
- 반드시 tbody.querySelectorAll(...).forEach(tr => tr.addEventListener('click', ...)) 방식 사용
- onclick은 goPage(3) 처럼 숫자 인자만 받을 때는 사용 가능

## 에러 가시화
- HTML body 최상단에 id="errorBox" div(기본 display:none)를 두고
- window.addEventListener('error', ...) 로 문법 오류까지 화면에 빨간 박스로 표시
- 각 렌더 함수를 try-catch로 감싸 어디서 실패했는지 바로 보이게

## 날짜/문자열 처리
- 모든 셀 값에 String(row[n] || '').trim() 적용 (앞뒤 공백/개행 제거)
- 잠재점수는 Number(row[n]) || 0
- 날짜 셀은 Utilities.formatDate(date, 'Asia/Seoul', 'yyyy-MM-dd') 로 문자열화하되, 날짜 비교용 원본은 별도 필드로 보존
